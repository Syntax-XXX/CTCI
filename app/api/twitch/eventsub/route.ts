...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if(moderation.blocked){
      await runAutomations(admin,profile.id,{type:'moderation_action',source:'twitch',text,userId:actor.userId,displayName:actor.displayName,metadata:{reason:moderation.reason||'',action:moderation.action||'hide'}})
      return new NextResponse(null,{status:204})
    }

    const engagement=await handleEngagementCommand(admin,profile.id,actor,text,overlay.command_prefix||'CC!')
    if(engagement.handled){
      if(engagement.message)await replyInTwitch(admin,profile.id,event.broadcaster_user_id,event.message_id,engagement.message)
      await mirrorToDiscord(admin,profile.id,event,text,true,timestamp)
      await runAutomations(admin,profile.id,{type:'command',source:'twitch',text,userId:actor.userId,displayName:actor.displayName,metadata:{kind:'engagement'}})
      return new NextResponse(null,{status:204})
    }

    let parsed = null
    try { parsed = parseCommand(text, overlay.command_prefix || 'CC!') } catch (error) {
      await admin.from('command_audit_log').insert({owner_id:profile.id,channel_twitch_user_id:event.broadcaster_user_id,twitch_user_id:event.chatter_user_id,twitch_login:actor.login,command_name:'parse',raw_command:text.slice(0,500),sanitized_args:[],source:'core',success:false,error_code:error instanceof Error?error.message:'parse_error'})
      return new NextResponse(null, { status: 204 })
    }
    if (parsed) {
      ...
      if (result.handled) {
        ...
        await mirrorToDiscord(admin, profile.id, event, text, true, timestamp)
        await runAutomations(admin,profile.id,{type:'command',source:'twitch',text,userId:actor.userId,displayName:actor.displayName,metadata:{command:parsed.command}})
        return new NextResponse(null, { status: 204 })
      }
    }
...
async function replyInTwitch(admin:any,ownerId:string,broadcasterId:string,parentMessageId:string,message:string){
  try{
    ...
    await sendTwitchChatMessage({accessToken:creds.access_token,broadcasterId,senderId:broadcasterId,message:message.slice(0,500),replyParentMessageId:parentMessageId})
  }catch(error){console.error('Twitch command reply failed',error)}
}
async function buildHelpMessage(admin:any,ownerId:string,prefix:string,arg?:string){const commands=listCommands();const{data:rows}=await admin.from('command_configurations').select('command_name,enabled,aliases,permissions,source,response_template').eq('owner_id',ownerId);const configs=new Map<string,any>((rows||[]).map((r:any)=>[String(r.command_name).toLowerCase(),r]));const custom=(rows||[]).filter((r:any)=>r.source==='custom'&&r.enabled!==false).map((r:any)=>String(r.command_name));const active=commands.filter(c=>configs.get(c.name)?.enabled!==false),q=(arg||'').toLowerCase();if(q&&!/^\d+$/.test(q)){const found=active.find(c=>c.name===q||c.aliases?.includes(q)||(configs.get(c.name)?.aliases||[]).includes(q));const customCfg=configs.get(q);if(customCfg?.source==='custom')return`${prefix}${q} — custom command`;if(!found)return`Unknown command. Try ${prefix}help`;const cfg=configs.get(found.name),roles=(cfg?.permissions?.length?cfg.permissions:found.permissions).join(', '),aliases=[...(found.aliases||[]),...(cfg?.aliases||[])].filter((v,i,a)=>a.indexOf(v)===i);return`${prefix}${found.name} — ${found.description} · roles: ${roles}${aliases.length?` · aliases: ${aliases.join(', ')}`:''}`.slice(0,500)}const names=[...active.map(c=>c.name),...custom.filter((v:string)=>!active.some(c=>c.name===v))],perPage=10,total=Math.max(1,Math.ceil(names.length/perPage)),page=Math.min(total,Math.max(1,Number(q||1)||1)),slice=names.slice((page-1)*perPage,page*perPage);return`Commands ${page}/${total}: ${slice.map((name:string)=>prefix+name).join(', ')}${page<total?` · ${prefix}help ${page+1}`:''}`.slice(0,500)}
