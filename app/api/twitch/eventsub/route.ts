...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (parsed) {
      ...
      if (result.handled) {
        let response = result.success ? result.message : undefined
...
async function buildHelpMessage(admin:any,ownerId:string,prefix:string,arg?:string){
  const commands=listCommands()
  const{data:rows}=await admin.from('command_configurations').select('command_name,enabled,aliases,permissions').eq('owner_id',ownerId)
  const configs=new Map((rows||[]).map((r:any)=>[String(r.command_name).toLowerCase(),r]))
  const active=commands.filter(c=>configs.get(c.name)?.enabled!==false)
  const q=(arg||'').toLowerCase()
  if(q && !/^\d+$/.test(q)){
    const found=active.find(c=>c.name===q||c.aliases?.includes(q)||(configs.get(c.name)?.aliases||[]).includes(q))
    if(!found)return `Unknown command. Try ${prefix}help`
    const cfg=configs.get(found.name),roles=(cfg?.permissions?.length?cfg.permissions:found.permissions).join(', '),aliases=[...(found.aliases||[]),...(cfg?.aliases||[])].filter((v,i,a)=>a.indexOf(v)===i)
    return `${prefix}${found.name} — ${found.description} · roles: ${roles}${aliases.length?` · aliases: ${aliases.join(', ')}`:''}`.slice(0,500)
  }
  const perPage=10,total=Math.max(1,Math.ceil(active.length/perPage)),page=Math.min(total,Math.max(1,Number(q||1)||1)),slice=active.slice((page-1)*perPage,page*perPage)
  return `Commands ${page}/${total}: ${slice.map(c=>prefix+c.name).join(', ')}${page<total?` · ${prefix}help ${page+1}`:''}`.slice(0,500)
}

async function mirrorToDiscord(admin:any,ownerId:string,event:any,text:string,isCommand:boolean){
  try{
    const{data:sync}=await admin.from('discord_chat_sync').select('channel_id,enabled,include_commands').eq('owner_id',ownerId).maybeSingle()
    if(!sync?.enabled||!sync.channel_id||(isCommand&&!sync.include_commands))return
    const name=escapeDiscord(String(event.chatter_user_name||event.chatter_user_login||'unknown'))
    const message=escapeDiscord(text).slice(0,1800)
    await sendDiscordChannelMessage(sync.channel_id,`🟣 **${name}**: ${message}`)
  }catch(error){console.error('Discord chat mirror failed',error)}
}
...
function getRoles(event:any){
  ...
  for(const badge of Array.isArray(event.badges)?event.badges:[]){const id=String(badge?.set_id||badge?.id||'').toLowerCase();if(id==='moderator')roles.add('moderator');if(id==='vip')roles.add('vip');if(id==='subscriber'||id==='founder')roles.add('subscriber')}
  return roles
}
