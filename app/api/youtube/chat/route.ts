...
export async function GET(request:NextRequest){
  ...
  try{
    const{data:overlay,error}=await admin.from('overlays').select('id,user_id,enabled,show_youtube_chat,youtube_auto_detect,youtube_video_id,youtube_live_chat_id,youtube_next_page_token,youtube_next_poll_at').eq('slug',slug).maybeSingle()
    ...
    if(overlay.youtube_auto_detect&&oauth){
      let liveChatId=String(overlay.youtube_live_chat_id||''),videoId=String(overlay.youtube_video_id||'')
      if(!liveChatId){
        ...
        videoId=active.id;liveChatId=active.liveChatId
        await admin.from('overlays').update({youtube_video_id:videoId,youtube_live_chat_id:liveChatId,youtube_active_title:active.title,youtube_last_discovery_at:new Date().toISOString(),youtube_next_page_token:null}).eq('id',overlay.id)
      }
...
async function persistChat(admin:any,overlay:any,videoId:string,liveChatId:string,response:any,autoDetect:boolean){const items=Array.isArray(response.items)?response.items:[];const rows=items.flatMap((item:any)=>{const snippet=item?.snippet||{},author=item?.authorDetails||{},type=String(snippet.type||'textMessageEvent'),text=messageText(snippet,type);if(!text||!item?.id)return[];return[{id:`youtube:${item.id}`,overlay_id:overlay.id,broadcaster_user_id:`youtube:${videoId}`,broadcaster_login:`youtube:${videoId}`,chatter_user_id:String(author.channelId||`youtube:${item.id}`),chatter_login:String(author.channelId||author.displayName||'youtube').toLowerCase(),chatter_name:String(author.displayName||'YouTube'),message_text:text,color:null,badges:youtubeBadges(author),fragments:[],reply:null,source:'youtube',event_type:eventType(type),event_data:eventData(snippet,type),created_at:String(snippet.publishedAt||new Date().toISOString())}]});if(rows.length){const{error}=await admin.from('chat_events').upsert(rows,{onConflict:'id',ignoreDuplicates:true});if(error)throw error}const interval=clamp(Number(response.pollingIntervalMillis)||5000,1000,30000),offline=!!response.offlineAt,update:any={youtube_live_chat_id:offline?null:liveChatId,youtube_video_id:offline&&autoDetect?null:videoId,youtube_next_page_token:offline?null:(response.nextPageToken||null),youtube_next_poll_at:new Date(Date.now()+(offline?10000:interval)).toISOString(),youtube_last_sync_at:new Date().toISOString()};if(offline)update.youtube_active_title=null;const{error}=await admin.from('overlays').update(update).eq('id',overlay.id);if(error)throw error;return NextResponse.json({ok:true,enabled:true,connected:!offline,autoDetect,videoId:offline?null:videoId,inserted:rows.length,nextPollMs:offline?10000:interval})}
