...
export async function GET(request:NextRequest){
  ...
  try{
    const{data:overlay,error}=await admin.from('overlays').select('id,user_id,enabled,show_youtube_chat,command_prefix,youtube_auto_detect,youtube_video_id,youtube_live_chat_id,youtube_next_page_token,youtube_next_poll_at').eq('slug',slug).maybeSingle()
...
async function persistChat(admin:any,overlay:any,videoId:string,liveChatId:string,response:any,autoDetect:boolean){
  ...
  for(const item of items){
    ...
    const userId=String(author.channelId||`youtube:${item.id}`),login=String(author.channelId||author.displayName||'youtube').toLowerCase(),displayName=String(author.displayName||'YouTube'),eventId=`youtube:${item.id}`
    const featureResult=await processChatFeatures({admin,ownerId:overlay.user_id,overlayId:overlay.id,source:'youtube',eventId,userId,login,displayName,text,commandPrefix:overlay.command_prefix||'CC!'})
    if(featureResult.handled||featureResult.suppressOverlay)continue
    rows.push({id:eventId,overlay_id:overlay.id,broadcaster_user_id:`youtube:${videoId}`,broadcaster_login:`youtube:${videoId}`,chatter_user_id:userId,chatter_login:login,chatter_name:displayName,message_text:text,color:null,badges:youtubeBadges(author),fragments:[],reply:null,source:'youtube',event_type:eventType(type),event_data:eventData(snippet,type),created_at:String(snippet.publishedAt||new Date().toISOString())})
  }
