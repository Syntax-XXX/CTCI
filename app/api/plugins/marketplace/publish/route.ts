...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    const existingVersion=await auth.admin.from('plugin_versions').select('id,checksum_sha256,marketplace_published').eq('plugin_id',m.id).eq('version',m.version).maybeSingle()
    if(existingVersion.error)throw existingVersion.error
    if(existingVersion.data?.marketplace_published&&existingVersion.data.checksum_sha256!==checksum){
      return NextResponse.json({error:`${m.id} v${m.version} is already published with different bytes. Bump the plugin version before publishing a changed package.`},{status:409})
    }
    ...
    if(!existingVersion.data){
      const version=await auth.admin.from('plugin_versions').insert({plugin_id:m.id,version:m.version,manifest:m,bundle_path:path,checksum_sha256:checksum,marketplace_published:true,marketplace_published_at:now}).select('id').single()
      if(version.error)throw version.error
    }else if(!existingVersion.data.marketplace_published){
      const version=await auth.admin.from('plugin_versions').update({manifest:m,bundle_path:path,checksum_sha256:checksum,marketplace_published:true,marketplace_published_at:now}).eq('id',existingVersion.data.id).select('id').single()
      if(version.error)throw version.error
    }
