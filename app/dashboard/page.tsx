...
export default function Dashboard() {
  ...
  useEffect(()=>{(async()=>{
    ...
    const {data,error}=await supabase.from('overlays').select('*').eq('user_id',user.id).single()
    if(error){setStatus(error.message);return}
    setOverlay({...defaults,...data} as Overlay);setStatus('Ready')
  })()},[router,supabase])
  ...
  async function saveChatter(){ const {data:{user}}=await supabase.auth.getUser(); if(!user||!chatter.chatter_login)return; const payload={...chatter,owner_id:user.id,chatter_login:chatter.chatter_login.toLowerCase()}; const {error}=await supabase.from('chatter_styles').upsert(payload,{onConflict:'owner_id,chatter_login'}); setStatus(error?error.message:'Chatter style saved') }
  ...
  return <main className="shell">
    <nav className="nav"><div className="brand">CTCI <span>Dashboard</span></div><div className="actions"><a className="btn" target="_blank" href={`/overlay/${overlay.slug}`}>Open OBS overlay</a><button className="btn" onClick={signOut}>Sign out</button></div></nav>
    <div className="grid">
      <section className="panel">
        <h2>Overlay settings</h2><p className="muted">Your OBS URL is <code>/overlay/{overlay.slug}</code></p>
        ...
        <div className="row"><div className="field"><label>Font family</label><input value={overlay.font_family} onChange={e=>patch('font_family',e.target.value)} /></div><div className="field"><label>Font size</label><input type="number" value={overlay.font_size} onChange={e=>patch('font_size',+e.target.value)} /></div></div>
        ...
        <hr style={{borderColor:'#2b2b36',margin:'28px 0'}} />
        <h2>Per-chatter style</h2><p className="muted">Override fonts and colors for specific chatters.</p>
        ...
        <div className="field"><label>Custom font family</label><input value={chatter.font_family} onChange={e=>setChatter({...chatter,font_family:e.target.value})} placeholder="e.g. Comic Sans MS"/></div>
        <div className="actions"><button className="btn" onClick={saveChatter}>Save chatter style</button></div>
      </section>
      <aside className="panel"><h2>Live preview</h2><div className="preview" style={previewStyle}><ChatPreview overlay={overlay}/></div><p className="small muted">Live Twitch ingestion requires Twitch OAuth/EventSub credentials. The OBS view and database are already configured.</p></aside>
    </div>
  </main>
}

function ChatPreview({overlay}:{overlay:Overlay}){const samples=[['LunaMod','This overlay is looking clean ✨'],['pixelPilot','Custom fonts per chatter? yes please'],['syntax_dev','OBS browser source is ready.']];return <>{samples.map(([u,m])=><div className="msg" key={u} style={{background:`rgba(${hex(overlay.bubble_color)},${overlay.bubble_opacity})`,borderRadius:overlay.border_radius,color:overlay.message_color,marginTop:overlay.message_spacing}}><strong style={{color:overlay.username_color}}>{u}</strong>{m}</div>)}</>}
